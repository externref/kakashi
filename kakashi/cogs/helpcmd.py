...
class MyHelp(HelpCommand):
    async def send_bot_help(self, mapping):
        ...
        for cog in categories:
            ...
            if cog.qualified_name in ["HelpCog", "Jishaku", "botbase"]:
                continue
            embed.add_field(
        ...
        embed.set_author(
            name=f"{self.context.bot.user.name.upper()} HELP",
    ...
    async def send_cog_help(self, cog: Cog):
        await self.context.reply(embed=await embed_for_cog(self, cog, self.context))
    ...
    async def send_command_help(self, command: Command):
        ...
        embed = Embed(
            description=desc, color=await EmbedColor.color_for(self.context.guild)
        )
        embed.set_author(
...
class HelpDropMenu(Select):
    ...
    async def callback(self, interaction: MessageInteraction):
        cog = [
            ...
            if interaction.values[0] == self.bot.get_cog(cog).qualified_name
        ]
        await interaction.message.edit(
            embed=await embed_for_cog(self, cog[0], interaction)
        )
        await interaction.response.send_message(
            f"Showing help for {cog[0].qualified_name.upper()} COMMANDS", ephemeral=True
        )
...
async def embed_for_cog(self, cog: Cog, inter: Union[Context, MessageInteraction]):
    ...
    embed = Embed(
        color=await EmbedColor.color_for(inter.guild), description=desc
    ).set_author(
        name=f"{cog.qualified_name.upper()} COMMANDS",
        icon_url=inter.guild.me.display_avatar,
    )
    embed.set_footer(
        text=f"Requested by : {inter.author}", icon_url=inter.author.display_avatar
    )
