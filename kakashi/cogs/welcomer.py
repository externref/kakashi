...
class Welcomer(Cog, name="welcome"):
    ...
    @command(name="welcome", hidden=True, description="Shows help for welcomer module")
    @bot_has_permissions(
    ...
    async def welcome_message(self, ctx: Context, *, message: str):
        ...
        await GuildDataBase.update_or_insert_guild_data(
            data[0], data[1], message, data[3], data[4]
        )
        await ctx.send(
    ...
    async def set_welcome_channel(self, ctx: Context, channel: Union[TextChannel, str]):
        ...
        if str(channel).lower() == "none":
            if not await GuildDataBase.get_guild_data(ctx.guild.id):
                ...
                await GuildDataBase.update_or_insert_guild_data(
                    ctx.guild.id, "none", data[2], data[3], data[4]
                )
                await ctx.reply(
                    embed=Embed(
        ...
        if data:
            await GuildDataBase.update_or_insert_guild_data(
                ctx.guild.id, channel.id, data[2], data[3], data[4]
            )
        else:
            await GuildDataBase.update_or_insert_guild_data(
                ctx.guild.id,
                channel.id,
                "$usermention , welcome to $server",
                "cyan",
                "embedmessage",
            )
        await ctx.reply(
    ...
    async def welcome_color(self, ctx: Context, color: str):
        ...
        """
        if not color in self.allowed_colors.keys():
            return await ctx.reply(
                embed=Embed(
                    title="INVALID COLOR",
                    color=Color.red(),
                    description=f'Available Colors : ```\n{",".join(c for c in self.allowed_colors.keys())}\n```',
                )
            )
        data = await GuildDataBase.get_guild_data(ctx.guild.id)
        if not data:
        ...
        await GuildDataBase.update_or_insert_guild_data(
            data[0], data[1], data[2], color, data[4]
        )
        await ctx.reply(
            embed=Embed(
                color=await EmbedColor.color_for(ctx.guild),
    ...
    async def change_welcome_type(self, ctx: Context, welcome_type: str):
        ...
        await GuildDataBase.update_or_insert_guild_data(
            data[0], data[1], data[2], data[3], f"{welcome_type}message"
        )
        await ctx.reply(
            embed=Embed(
    ...
    async def on_member_join(self, member: Member):
        ...
        if data[4] == "textmessage":
            ...
            embed = Embed(
                description=welcome_message, color=self.allowed_colors[data[3]]
            ).set_author(
                name=str(member), icon_url=(member.avatar or member.default_avatar).url
            )
    ...
    async def process_message(self, message, member):
        message_to_return = (
            ...
            .replace(
                "$joined-discord-timestamp", str(int(member.created_at.timestamp()))
            )
