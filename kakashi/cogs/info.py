...
class Info(Cog, name="info"):
    ...
    def __init__(self, bot: Bot):
        self.bot = bot
        self.help_desc = (
            "Various Information commands for you to keep a check on your server"
        )
    ...
    async def send_avatar(self, ctx: Context, user: User = None):
        ...
        await ctx.reply(
            ...
            .set_image(url=(user.avatar or user.default_avatar).url)
            .set_footer(
                text=f"Requested by {ctx.author}", icon_url=ctx.author.display_avatar
            )
            .set_author(name=f"{user}' AVATAR")
        )
    ...
    async def get_information_about(self, ctx: Context, target: Union[Member, Role]):
        ...
        if isinstance(target, Member):
            embed = await EmbedMaker.for_member(ctx, target)
        elif isinstance(target, Role):
            embed = await EmbedMaker.for_role(ctx, target)
    ...
    async def send_userinfo(self, ctx: Context, member: Member = None):
        ...
        await ctx.reply(embed=await EmbedMaker.for_member(ctx, member))
    ...
    async def send_roleinfo(self, ctx: Context, role: Role):
        ...
        Info about a role
        """
        await ctx.reply(embed=await EmbedMaker.for_role(ctx, role))

    @command(name="serverinfo", aliases=["si", "svinfo"])
    @guild_only()
    @bot_has_permissions(
        send_messages=True, embed_links=True, read_message_history=True
    )
    async def guild_info(self, ctx: Context):
        ...
        """
        await ctx.reply(embed=await EmbedMaker.for_guild(ctx))
...
class EmbedMaker:
    async def for_member(
        ctx: Union[Context, ApplicationCommandInteraction], member: Member
    ) -> Embed:
    ...
    async def for_role(
        ctx: Union[Context, ApplicationCommandInteraction], role: Role
    ) -> Embed:
        info_dict = {
            "Name": role.name,
            "ID": role.id,
        ...
        if (
            ...
            info_dict["Perms"] = " , ".join(
                permission[0] for permission in role.permissions if permission[1]
            )
        ...
        embed = Embed(
            description=desc, color=await EmbedColor.color_for(ctx.guild)
    ...
    async def for_guild(ctx: Union[Context, ApplicationCommandInteraction]) -> Embed:
        info_dict = {
            ...
            "Boosters | Boost Level": " | ".join(
                [str(ctx.guild.premium_subscription_count), str(ctx.guild.premium_tier)]
            ),
        }
        if ctx.guild.icon:
        ...
        embed = Embed(
            color=await EmbedColor.color_for(ctx.guild), description=info_data
        ...
        if ctx.guild.banner:
            embed.set_thumbnail(url=ctx.guild.banner.url)
        return embed
